...
- (void)cutOkBtnClick
{
    if(self.confirmCutBlock){
        self.confirmCutBlock(_cutView.frame);
    }
    NSLog(@"裁剪的frame%@",_cutView);
}
...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    switch (_moveCorner) {
        ...
        case 6:
        {
            ...
            NSLog(@"frame,%f",offsetY);
            NSLog(@"%f",_cutView.frame.origin.y + offsetY);
            NSLog(@"%f",_cutView.frame.origin.y + _cutView.frame.size.height);
            CGRect frame = _cutView.frame;
            
            if(((_cutView.frame.origin.y + offsetY) < STATUS_BAR_HEIGHT))return;
            
            if(frame.size.height - offsetY <= 0) return;
            if(((_cutView.frame.origin.y + offsetY) > (_cutView.frame.origin.y + _cutView.frame.size.height ) ))return;
            
            
            frame.size.height = _cutView.frame.size.height - offsetY ;
            
            frame.origin.y = _cutView.frame.origin.y + offsetY ;
            if((_cutView.frame.origin.y < fabs(offsetY)))return;
            if((_cutView.frame.origin.y < fabs(offsetY)))return;
            _cutView.frame = frame;
            [_cutView sizeFitFourCorner];
            [self changeShadowWithFrame];
        }
            break;
        case 7:
        {
            ...
            if(frame.origin.y + frame.size.height + offsetY >= kHeight) return;
            if(((_cutView.frame.origin.y + offsetY) > (_cutView.frame.origin.y + _cutView.frame.size.height ) ))return;
        ...
        case 8:
        {
          ...
          // 做限制判断
            ...
            NSLog(@"frame,%f",offsetX);
            NSLog(@"%f",_cutView.frame.origin.x + offsetX);
            NSLog(@"%f",_cutView.frame.origin.x + _cutView.frame.size.width);
            CGRect frame = _cutView.frame;
            
           
            if(frame.size.width - offsetX <= 0) return;
            if(((_cutView.frame.origin.x + offsetX) > (_cutView.frame.origin.x + _cutView.frame.size.width ) ))return;
            
            
            frame.size.width = _cutView.frame.size.width - offsetX;
            
            frame.origin.x = _cutView.frame.origin.x + offsetX ;
            if((_cutView.frame.origin.x < fabs(offsetX)))return;
            if((_cutView.frame.origin.x < fabs(offsetX)))return;
            _cutView.frame = frame;
            [_cutView sizeFitFourCorner];
            [self changeShadowWithFrame];
...
- (int)judgeIsFourCornerTouch:(CGPoint)point
{
    ...
    if (x > [self getFirstCorner].x - 50 && x < [self getFirstCorner].x + 50 && y > [self getFirstCorner].y - 50 && y < [self getFirstCorner].y + 50) {
        ...
        return 5;
    } else if(x > (_cutView.frame.origin.x ) && x < (_cutView.frame.origin.x +   _cutView.frame.size.width ) && y > (_cutView.frame.origin.y - 50) && y < (_cutView.frame.origin.y + 50) ){                                             //上
        return 6;
    } else if(x > (_cutView.frame.origin.x ) && x < (_cutView.frame.origin.x +   _cutView.frame.size.width )  && y > (_cutView.frame.origin.y + _cutView.frame.size.height - 50) && y < (_cutView.frame.origin.y + _cutView.frame.size.height + 50)){                                             //下
        return 7;
    } else if(x > (_cutView.frame.origin.x - 50 ) && x < (_cutView.frame.origin.x + 50 )  && y > (_cutView.frame.origin.y ) && y < (_cutView.frame.origin.y + _cutView.frame.size.height )){                                             //左
        return 8;
    }  else if(x > (_cutView.frame.origin.x + _cutView.frame.size.width - 50 ) && x < (_cutView.frame.origin.x + _cutView.frame.size.width + 50 )  && y > (_cutView.frame.origin.y ) && y < (_cutView.frame.origin.y + _cutView.frame.size.height )){                                             //右
        return 9;
    }
